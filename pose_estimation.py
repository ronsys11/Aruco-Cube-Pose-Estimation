...
class ArUcoDetector:
    def __init__(self):
        ...
        self.offset = 0.002 # difference between marker size and cube size / 2
        self.marker_size = 0.04 + self.offset  # Marker size: 5 cm      
        c_pt = self.marker_size / 2
        # Define the ArUco board parameters
        board_ids = np.array([[2], [1], [4], [8], [5]], dtype=np.int32)
        board_corners = [
    ...
    def image_callback(self, msg):
        ...
        if self.camera_matrix is not None and self.dist_coeffs is not None:
            ...
            cv2.putText(cv_image, f'x: {tvec[0][0]*100:.2f} cm y: {tvec[1][0]*100:.2f}cm z: {tvec[2][0]*100:.2f} cm', (20, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
            cv2.imshow("ArUco Detection", cv_image)
            cv2.waitKey(1)
        else:
            # If camera matrix and distortion coefficients are not yet available, just display the raw image
            cv2.imshow("ArUco Detection", cv_image)
            cv2.waitKey(1)
